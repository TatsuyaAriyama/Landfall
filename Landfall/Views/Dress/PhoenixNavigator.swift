...
enum PhoenixNavigator {
    ...
    /// Imported Blender cape wind.  All cape meshes share cape-local Blender
    /// coordinates, so cloth, stitches, star and metal tips travel together.
    private static let importedCapeWindShader = """
    #pragma arguments
    float capeWind;
    #pragma body
    float x = _geometry.position.x;
    float z = _geometry.position.z;
    float falloff = clamp((0.12 - z) / 0.84, 0.0, 1.0);
    float flutter = pow(falloff, 1.38) * capeWind;
    float travelling = sin(scn_frame.time * 2.05 - z * 8.2 + x * 3.8);
    float crosswind = sin(scn_frame.time * 1.31 + x * 8.5 + z * 2.7);
    _geometry.position.x += crosswind * 0.008 * flutter;
    _geometry.position.y += (travelling * 0.048 + crosswind * 0.022) * flutter;
    _geometry.position.z += sin(scn_frame.time * 1.47 + x * 5.4) * 0.008 * flutter;
    """

    private static func applyImportedCapeWind(to imported: SCNNode) {
        guard let cape = imported.childNode(withName: "cape", recursively: true) else { return }
        cape.enumerateChildNodes { node, _ in
            ...
            geometry.materials = geometry.materials.map { source in
                ...
                modifiers[.geometry] = importedCapeWindShader
                material.shaderModifiers = modifiers
                material.isDoubleSided = true
                material.setValue(NSNumber(value: 1.0), forKey: "capeWind")
                return material
            }
    ...
    private static func makeLegacyNavigatorNode() -> SCNNode {
        let root = SCNNode()
        root.name = "navigator"
...
final class PhoenixAnimator: NSObject, SCNSceneRendererDelegate {
    ...
    private var capeMaterials: [SCNMaterial] = []
    ...
    private func bind(_ scene: SCNScene) {
        ...
        capeMaterials.removeAll(keepingCapacity: true)
        cape?.enumerateChildNodes { [weak self] node, _ in
            guard let self, let geometry = node.geometry else { return }
            self.capeMaterials.append(contentsOf: geometry.materials)
        }
    ...
    func step(t: Float, dt: Float) {
        ...
        if let cape {
            cape.eulerAngles.x = -0.012 * wind + sin(t * 0.74) * 0.008
            cape.eulerAngles.z = sin(t * 0.58) * 0.006 * wind
        }
        for material in capeMaterials {
            material.setValue(NSNumber(value: wind), forKey: "capeWind")
        }
