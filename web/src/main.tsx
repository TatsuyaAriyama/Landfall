...
const APP_BUILD = "2026-08-10-ios-home-navigation-v1";
...
function installLazyLoadRecovery() {
  ...
  window.addEventListener("vite:preloadError", (event) => {
    ...
    // URLのパスだけを控え、旧Service Worker移行端末でどの分割JSが失敗したか
    // DevToolsから判断できるようにする。クエリや設定値はDOMへ出さない。
    const failedUrl = failedAssetUrl(payload);
    if (failedUrl) {
      document.documentElement.dataset.preloadError = new URL(failedUrl).pathname;
    } else if (payload instanceof Error) {
      document.documentElement.dataset.preloadError = payload.name;
      document.documentElement.dataset.preloadErrorDetail = payload.message
        .replace(/https?:\/\/\S+/g, "[url]")
        .slice(0, 160);
    } else {
      document.documentElement.dataset.preloadError = typeof payload;
    }
